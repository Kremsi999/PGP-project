...
def import_private_key(email):
    HasPass = ""
    pattern = re.compile(r"\w+\s\w+" + re.escape(email) + r"_private_key_dsa_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    pattern1 = re.compile(r"\w+\s\w+" + re.escape(email) + r"_private_key_dsa_\w{1}_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
...
def import_public_key(email):
    pattern = re.compile(r"\w+\s\w+" + re.escape(email) + r"_public_key_dsa_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")

...
def generatePrivateRing(email, password=None):
    ...
    ruta1 = re.compile(r"\w+\s\w+" + re.escape(email) +
                         r"_private_key_rsa_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    private_key_path = None

    ruta2 = re.compile(r"\w+\s\w+" + re.escape(email) +
                          r"_private_key_rsa_\w{1}_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    ruta3 = re.compile(r"\w+\s\w+" + re.escape(email) + r"_private_key_dsa_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    ruta4 = re.compile(
        r"\w+\s\w+" + re.escape(email) + r"_private_key_dsa_\w{1}_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    ...
    for filename in os.listdir("Kljucevi"):
        ...
        if ruta1.match(filename) or ruta2.match(filename) or ruta3.match(filename) or ruta4.match(filename):
            ...
            if match:
                HasPass = match[0]
                datetime_str = re.findall(r"\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}", private_key_path)
            else:
                HasPass = ""
                datetime_str = re.findall(r"\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}", private_key_path)
            ...
            datetime_str = re.findall(r"\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}", private_key_path)
            if datetime_str:
                datetime_obj = datetime.strptime(datetime_str[0], "%Y-%m-%d_%H:%M:%S")
...
def generatePublicRing():
    ...
    pattern = re.compile(r"\w+\s\w+\ˆ[-A-Za-z0-9!#$%&'*+/=?^_`{|}~]+(?:\.[-A-Za-z0-9!#$%&'*+/=?^_`{|}~]+)*@(?:[A-Za-z0-9](?:[-A-Za-z0-9]*[A-Za-z0-9])?\.)+[A-Za-z0-9](?:[-A-Za-z0-9]*[A-Za-z0-9])?_public_key_rsa_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    pattern1 = re.compile(r"\w+\s\w+\ˆ[-A-Za-z0-9!#$%&'*+/=?^_`{|}~]+(?:\.[-A-Za-z0-9!#$%&'*+/=?^_`{|}~]+)*@(?:[A-Za-z0-9](?:[-A-Za-z0-9]*[A-Za-z0-9])?\.)+[A-Za-z0-9](?:[-A-Za-z0-9]*[A-Za-z0-9])?_public_key_dsa_\w{1}_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
    ...
    for filename in os.listdir("Kljucevi"):
        if pattern.match(filename) or pattern1.match(filename):
            ...
            datetime_str = re.findall(r"\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}", public_key_path)
            ...
            KeyID = public_key.public_bytes(encoding=serialization.Encoding.PEM,
                                                          format=serialization.PublicFormat.SubjectPublicKeyInfo)[-34:-26]
            print(KeyID)
            KeyID = base64.b64encode(KeyID).decode()
            KeyID = int.from_bytes(base64.urlsafe_b64decode(KeyID), 'big')

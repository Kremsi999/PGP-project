...
def generate_elgamal_keys(name, email, key_size, password=None):
    ...
    with open(f'Kljucevi/{name}_{email}_public_key_elgamal_{key_size}_{timestamp}.pem', 'wb') as f:
        f.write(base64.b64encode(str(p).encode()))
        f.write(b'\n')
        f.write(base64.b64encode(str(g).encode()))
        f.write(b'\n')
...
def import_privte_elgamal_keys_from_pem(email):
    pattern = re.compile(r"\w+\s\w+" + re.escape(email) +
                         r"_private_key_elgamal_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
...
def import_public_elgamal_keys_from_pem(email):
    pattern = re.compile(
        r"\w+\s\w+" + re.escape(email) + r"_public_key_elgamal_\d{4}_\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}.pem")
...
#
# PR = import_privte_elgamal_keys_from_pem("nemanja@gmail.com")
# PU = import_public_elgamal_keys_from_pem("nemanja@gmail.com")
#
# plaintext = 42
#
# # Encryption
